const appArea = new (function () {
    ...
    this.guardarArea = () => {
        ...
        fetch("../controllers/guardarArea.php", { method: "POST", body: formArea })
            ...
            .then((data) => {
                if (data == true) {
                    app.notificacion('¡Área guardada!', 'Se ha agregado una nueva área.', 'guardar');
                    this.listadoArea();
                    this.limpiarArea();
                } else {
                ...
                data.forEach(item => {
                    ...
                    if (editarCategoria) {
                        ...
                        editarCategoria.innerHTML += `
                            <option ${seleted} value="${item.id_categoria}">${item.detalle}</option>
                        `;
                    } else {
                        this.categoria.innerHTML += `
                            <option value="${item.id_categoria}">${item.detalle}</option>
                        `;
                    }
                });
            })
                    ...
                    for (let i = 0; i < item.length; i++) {
                        html += `
                            <tr class="h-16 border-b last:border-b-0 border-b-white-100">   
                                <td class="pr-4">${item[i].id_area}</td>
                                <td class="pr-4">${item[i].area_detalle}</td>
                                <td class="pr-4">
                                ...
                                <td class="flex justify-end flex-row items-center gap-4 h-16 w-fit ml-auto">
                                    <button onclick="appArea.editarArea(${item[i].id_area})" title="Editar" class="btn-editar flex items-center gap-2 min-h-fit rounded-md bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                        </svg>
                                    </button>
                                    <button onclick="appArea.eliminarArea(${item[i].id_area})" title="Anular" class="${item[i].area_estado === 'anulado' ? 'hidden ' : ''}btn-eliminar flex items-center gap-2 min-h-fit rounded-md bg-red-50 px-3 py-2 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">
    ...
    this.paginacionArea = (pagina_actual, total_paginas, buscador) => {
        ...
        if (buscador == false) {
            ...
            if (pagina_actual > 1) {
                html += `
                    <a href="javascript:appArea.listadoArea(${pagina_actual - 1})" class="btn-anterior border border-slate-200 bg-slate-100 color-slate-600 p-4 rounded-md">Anterior</a>
                `;
            }
            if (total_paginas > 1) {
                html += `
            ...
            if (pagina_actual < total_paginas) {
                html += `
                    <a href="javascript:appArea.listadoArea(${pagina_actual + 1})" class="btn-siguiente border border-slate-200 bg-slate-100 color-slate-600 p-4 rounded-md">Siguiente</a>
                `;
            }
        } else {
            ...
            if (pagina_actual > 1) {
                html += `
                    <a href="javascript:appArea.busquedaArea(${pagina_actual - 1})" class="btn-anterior border border-slate-200 bg-slate-100 color-slate-600 p-4 rounded-md">Anterior</a>
                `;
            }
            if (total_paginas > 1) {
                html += `
            ...
            if (pagina_actual < total_paginas) {
                html += `
                    <a href="javascript:appArea.busquedaArea(${pagina_actual + 1})" class="btn-siguiente border border-slate-200 bg-slate-100 color-slate-600 p-4 rounded-md">Siguiente</a>
                `;
            }
        }
    ...
    this.editarArea = (id_area) => {
        ...
        fetch("../controllers/editarArea.php", { method: "POST", body: formArea })
            ...
            .then((data) => {
                ...
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 overflow-y-auto z-50 ease-out duration-300">
                    <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                        <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-5xl">
                            <form action="javascript:void(0);" onsubmit="appArea.actualizarArea()">
                                ...
                                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                        ...
                                        <div class="sm:col-span-2">
                                            ...
                                            <input type="text" id="editar-detalle-area" value="${data.detalle}" class="block w-full mt-2 rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6">
                                        </div>
                                        <!-- Categoria -->
                                        <div class="sm:col-span-2">
                                            <label class="block text-sm font-medium leading-6 text-gray-900">Categoría:</label>
                                            <select id="editar-categoria-area" required class="categoria-id-${data.id_categoria_fk} h-[38px] block w-full mt-2 rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm sm:leading-6">
                                        ...
                                        <div class="sm:col-span-2">
                                            ...
                                            <select id="editar-estado-area" required class="h-[38px] block w-full mt-2 rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm sm:leading-6">
                                                <option disabled>-- Selecciona --</option>
                                ...
                                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    ...
                                    <button type="button" onclick="appArea.cerrarModalArea()" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancelar</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                $('#editar-categoria-area').select2();
                $('#editar-estado-area').select2();
        ...
        if (confirmar == true) {
            ...
            fetch("../controllers/eliminarArea.php", { method: "POST", body: formArea })
                ...
                .then((data) => {
                    ...
                    app.notificacion('¡Área eliminada!', 'Se ha eliminado una área.', 'eliminar');
                    this.listadoArea();
                    this.busqueda.value = null;
                })
    ...
    this.limpiarArea = () => {
        // Jquery para trabajar con la biblioteca Select2
        // variables
        var selectCategoria = $('#categoria-area');
        // resetear opciones
        this.detalle.value = '';
        selectCategoria.empty().append('<option selected disabled>-- Selecciona --</option>');
    }
    this.cerrarModalArea = () => {
        var modalArea = document.getElementById('modal-area');
        modalArea.innerHTML = '';
