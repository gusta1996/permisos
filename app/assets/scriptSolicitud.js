const appSolicitud = new (function () {
                        ...
                        <tr class="h-16 border-b last:border-b-0 border-b-white-100">
                            ...
                            <td class="pr-4">
                                <div class="flex justify-end flex-row items-center gap-4 h-16 w-fit">
                                    ...
                                    <div title="${item[i].firma_autorizador == true ? 'Firmado por el jefe de dirección' : 'Firma del jefe de dirección pendiente'}" class="cursor-help flex items-center gap-2 min-h-fit rounded-md px-3 py-2 text-xs font-medium bg-${color_class_FA}-50 text-${color_class_FA}-700 ring-${color_class_FA}-600/10 ring-1 ring-inset">
    ...
    this.listadoSolicitudAutorizador = (pagina) => {
        // al cargar la pagina, la variable pagina es igual 1
        var pagina = !pagina ? 1 : pagina;
        var cuentaIdFuncionario = this.cuentaIdFuncionario.innerHTML;
        fetch("../controllers/listadoSolicitud.php?page=" + pagina + "&id=" + cuentaIdFuncionario + "&rol=autorizador")
            .then(response => response.json())
            .then(data => {
                // Datos de la lista y total de paginas
                let item = data.resultado;
                    ...
                    for (let i = 0; i < item.length; i++) {
                        ...
                        <tr class="h-16 border-b last:border-b-0 border-b-white-100">
                            ...
                            <td class="flex justify-end flex-row items-center gap-4 h-16 w-fit ml-auto">
                                ...
                                ` : ``}
                                ${(item[i].firma_estandar == false && item[i].fs_estado == 'pendiente') ? `
    ...
    this.listadoSolicitudSimple = (pagina) => {
        ...
        var pagina = !pagina ? 1 : pagina;
        var cuentaIdFuncionario = this.cuentaIdFuncionario.innerHTML;
        fetch("../controllers/listadoSolicitud.php?page=" + pagina + "&id=" + cuentaIdFuncionario + "&rol=estandar")
            .then(response => response.json())
            .then(data => {
                // Datos de la lista y total de paginas
                let item = data.resultado;
                            ...
                            <tr class="h-16 border-b last:border-b-0 border-b-white-100">
                                ...
                                <td class="pr-4">
                                    <div class="flex justify-end flex-row items-center gap-4 h-16 w-fit">
                                        ...
                                        <!-- Icono firma Autorizador -->
                                        <div title="${item[i].firma_autorizador == true ? 'Firmado por el jefe de dirección' : 'Firma del jefe de dirección pendiente'}" class="flex items-center gap-2 min-h-fit rounded-md px-3 py-2 text-xs font-medium bg-${color_class_FA}-50 text-${color_class_FA}-700 ring-${color_class_FA}-600/10 ring-1 ring-inset">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-badge" viewBox="0 0 16 16">
                                                <path d="M6.5 2a.5.5 0 0 0 0 1h3a.5.5 0 0 0 0-1zM11 8a3 3 0 1 1-6 0 3 3 0 0 1 6 0"/>
                                                <path d="M4.5 0A2.5 2.5 0 0 0 2 2.5V14a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2.5A2.5 2.5 0 0 0 11.5 0zM3 2.5A1.5 1.5 0 0 1 4.5 1h7A1.5 1.5 0 0 1 13 2.5v10.795a4.2 4.2 0 0 0-.776-.492C11.392 12.387 10.063 12 8 12s-3.392.387-4.224.803a4.2 4.2 0 0 0-.776.492z"/>
                                            </svg>
    ...
    this.busquedaSolicitudAutorizador = (pagina) => {
        // al cargar la pagina, la variable pagina es igual 1
        var pagina = !pagina ? 1 : pagina;
        // Si existe texto de busqueda, buscar
        var text = this.busqueda.value;
    ...
    this.paginacionSolicitud = (pagina_actual, total_paginas, buscador) => {
        ...
        if (this.tablaCompleta) {
            if (this.tablaCompleta.classList[0] == 'tabla-autorizador') {
                var listadoSolicitud = 'listadoSolicitudAutorizador';
                var busquedaSolicitud = 'busquedaSolicitudAutorizador';
            } else {
                var listadoSolicitud = 'listadoSolicitudCompleta';
                var busquedaSolicitud = 'busquedaSolicitud';
            }
        } else if (this.tablaSimple) {
            var listadoSolicitud = 'listadoSolicitudSimple';
            var busquedaSolicitud = 'busquedaSolicitud';
        }
        if (buscador == false) {
...
if (tablaAdministrador || tablaValidador) {
    ...
    appSolicitud.listadoSolicitudAutorizador();
