const appSolicitud = new (function () {
    ...
    this.btnSolicitud = document.getElementById('btn-solicitud');
    this.boxListaSolicitud = document.getElementById('lista-solicitud');
    this.boxCrearSolicitud = document.getElementById('crear-solicitud');
    this.funcionarioSolicitud = document.getElementById('funcionario-solicitud');
    ...
    this.listadoSolicitud = () => {
        if( this.tbodySolicitud ) {
            fetch("../controllers/listadoFuncionarioSolicitud.php")
                .then((resultado) => resultado.json())
                .then((data) => {
                    this.tbodySolicitud.innerHTML = '';
                    data.forEach(item => {
                        var id_funcionario_solicitud = item[0];
                        var estadoFS = item[1];
                        var id_funcionario = item[2];
                        var nombres = item[3];
                        var apellidos = item[4];
                        var numero = item[5];
                        var fecha = item[6];
                        var razon = item[7];
                        this.tbodySolicitud.innerHTML += `
                        <tr class="h-14 border-b last:border-b-0 border-b-white-100">
                            <td class="hidden text-slate-700 pr-4">${id_funcionario}</td>
                            <td class="text-slate-700 pr-4">${numero}</td>
                            <td class="text-slate-700 pr-4">${apellidos} ${nombres}</td>
                            <td class="text-slate-700 pr-4">${razon}</td>
                            <td class="text-slate-700 pr-4 whitespace-nowrap">${fecha}</td>
                            <td class="${estadoFS == 'Anulado' ? 'text-red-600' : (estadoFS == 'Pendiente' ? 'text-amber-400' : 'text-slate-700')} pr-4">${estadoFS}</td>
                            <td class="flex justify-end flex-row items-center gap-4 h-14 w-fit ml-auto">
                                <button onclick="appSolicitud.editarSolicitud(${id_funcionario_solicitud})" class="btn-editar flex items-center gap-2 min-h-fit rounded-md bg-blue-50 px-3 py-2 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-700/10">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                        <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                        <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                    </svg>
                                </button>
                                <button onclick="appSolicitud.aprobarSolicitud(${id_funcionario_solicitud})" class="${estadoFS != 'Pendiente' ? 'hidden ' : ''}btn-aprobar flex items-center gap-2 min-h-fit rounded-md bg-green-50 px-3 py-2 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/10">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                                        <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z"/>
                                    </svg>
                                </button>
                                <button onclick="appSolicitud.eliminarSolicitud(${id_funcionario_solicitud})" class="${estadoFS === 'Anulado' ? 'hidden ' : ''}btn-eliminar flex items-center gap-2 min-h-fit rounded-md bg-red-50 px-3 py-2 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">
    ...
    this.selectFuncionarios = () => {
        fetch("../controllers/listadoFuncionarioEstructura.php")
            .then((resultado) => resultado.json())
            .then((data) => {
                data.forEach(item => {
                    var editarFuncionario = document.getElementById('editar-funcionario-solicitud');
                    if (editarFuncionario) {
                        var selected = 'funcionario-id-' + item.id_funcionario_estructura == editarFuncionario.classList.item(0) ? 'selected' : '';
                        editarFuncionario.innerHTML += `
                        <option ${selected} value="${item.id_funcionario_estructura}">${item.apellidos} ${item.nombres}</option>
                    `;
                    } else if (this.funcionarioSolicitud) {
    ...
    this.guardarSolicitud = () => {
        ...
        formSolicitud.append('id_funcionario', this.funcionarioSolicitud.value);
        fetch("../controllers/obtenerIdFunEstruc.php", { method: "POST", body: formSolicitud })
            .then((respuesta) => respuesta.json())
            .then((data) => {
                var id_funcionario_estructura = data.id_funcionario_estructura;

                formSolicitud.append('id_funcionario_estructura', id_funcionario_estructura);
    ...
    this.actualizarSolicitud = () => {
        ...
        var funcionarioSolicitud = document.getElementById('editar-funcionario-solicitud');
        ...
        formSolicitud.append('id_funcionario', funcionarioSolicitud.value);
        ...
        formSolicitud.append('estado', editarEstado.value);
    ...
    this.editarSolicitud = (id_funcionario_solicitud) => {
        var formSolicitud = new FormData();
                                ...
                                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    <div class="mt-2 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                        ...
                                        <div class="col-span-full">
                                            ...
                                            <div class="mt-2">
                                                <select id="editar-funcionario-solicitud" required class="funcionario-id-${data.id_funcionario_estructura} h-[38px] block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm sm:leading-6">
                                        ...
                                        <div class="sm:col-span-3">
                                            ...
                                            <div class="mt-2">
                                                <input type="text" id="editar-razon-solicitud" value="${data.descripcion}" required class="block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6">
                                            </div>
                                        </div>
                                        <!-- Estado -->
                                        <div class="sm:col-span-3">
    ...
    this.btnCrearSolicitud = () => {
        this.btnSolicitud.remove();
        this.boxListaSolicitud.classList.add('hidden');
        this.boxCrearSolicitud.classList.remove('hidden');
    }
});
appSolicitud.listadoSolicitud();